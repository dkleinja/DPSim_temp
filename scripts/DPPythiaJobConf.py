...
class DPSimJobConf:
    ...
    def __init__(self, template, reserved = []):
        ...
        self.attr = {}
        self.contents = []
        self.userdef = ['seed', 'outputFileName','externalInput'] + reserved
    ...
    def generate(self, confname, seed = 1, outputName = '', externalInput = '', reserved = {}):

        if outputName == '':
            outputName = confname.replace('.conf', '.root')

        if externalInput== '':
            externalInput = confname.replace('.conf', '.root')
